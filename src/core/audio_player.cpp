...
std::unique_ptr<PacketQueue> AudioPlayer::s_AudioPacketQueue =
...
int AudioPlayer::init_swr_ctx(AVFrame* av_frame, SampleRate sample_rate) {
  ...
  swr_alloc_set_opts(s_Resampler_Context, channel_layout, AV_SAMPLE_FMT_FLT,
                     sample_rate.first, channel_layout, AV_SAMPLE_FMT_FLTP,
                     sample_rate.second, 0, nullptr);
  ...
  std::cout << "Failed to initialize the resampler context: "
            << av_error_to_string(init_result) << "\n";
...
int AudioPlayer::update_audio_stream(AudioState* userdata, Uint8* sdl_stream,
                                     int& len) {
  ...
  const auto out_samples =
      swr_get_out_samples(s_Resampler_Context, num_samples);

  const AVSampleFormat sample_format = userdata->av_codec_ctx->sample_fmt;

  auto buffer_size = av_samples_get_buffer_size(nullptr, num_channels,
                                                num_samples, sample_format, 1);
  ...
  if (!av_sample_fmt_is_planar(sample_format)) {
    if (synchronize_audio(userdata,
                          reinterpret_cast<float*>(s_LatestFrame->data[0]),
    ...
    const int sample_per_ch_size =
        nonplanar_sample_size * userdata->av_codec_ctx->channels;

    const int bytes_per_sec =
        sample_per_ch_size * userdata->av_codec_ctx->sample_rate;
  ...
  AudioResamplingState resampling_data = {&resampled_audio_buffer, num_samples,
                                          num_channels, out_samples};
  ...
  static const auto bytes_per_sample =
      sizeof(decltype(resampled_audio_buffer)::value_type);

  const int sample_per_ch_size =
      bytes_per_sample * userdata->av_codec_ctx->channels;
...
int AudioPlayer::add_dummy_samples(float* samples, int* samples_size,
...
int AudioPlayer::synchronize_audio(struct AudioState* audio_state,
  ...
  audio_state->delta_accum =
      delta + AUDIO_DIFF_AVG_COEF * audio_state->delta_accum;
  ...
  int wanted_size = *samples_size + (static_cast<int>(delta * sample_rate) *
                                     total_sample_bytes);
  ...
  add_dummy_samples(samples, samples_size, wanted_size, max_size,
                    total_sample_bytes);
...
int AudioPlayer::decode_audio_packet(struct AudioState* userdata,
...
void AudioPlayer::resample_audio(AVFrame* frame,
                                 struct AudioResamplingState data) {
  if (!s_Resampler_Context) {
  ...
  int ret = swr_convert(s_Resampler_Context,
