...
namespace YAVE {
Debugger::Debugger() : video_state(nullptr) {}
...
void Debugger::render() {
  ...
  const std::string video_pts =
      "Current Video PTS: " + std::to_string(video_state->pts) + " sec";

  const std::string video_internal_clock =
      "Video Internal Clock: " + std::to_string(AudioPlayer::get_master_clock()) +
      " sec";

  const std::string audio_internal_clock =
      "Audio Internal Clock: " +
      std::to_string(AudioPlayer::get_audio_internal_clock()) + " sec";
  ...
  diff_sum +=
      AudioPlayer::get_master_clock() - AudioPlayer::get_audio_internal_clock();

  count_sample++;

  const std::string clock_diff =
      "Average Clock Difference: " + std::to_string(diff_sum / count_sample) +
      " sec";

  const int kilobytes_per_second = calculate_kilobytes_per_second();

  const std::string kilobytes_per_second_str =
      "Kilobytes Processed per Second: " +
      std::to_string(kilobytes_per_second) + " kb";

  // Video Information
  const std::string width_str =
      "Width: " + std::to_string(video_state->dimensions.x) + "px";
  const std::string height_str =
      "Height: " + std::to_string(video_state->dimensions.y) + "px";

  // Audio Information
  const float sample_rate =
      static_cast<float>(AudioPlayer::s_AudioBufferInfo->sample_rate) / 1000.0f;

  const std::string sample_rate_str =
      "Sample Rate: " + std::to_string(sample_rate) + " kHz";
