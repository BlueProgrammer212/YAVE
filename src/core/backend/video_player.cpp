...
int VideoPlayer::create_context_for_stream(StreamInfoPtr& stream_info) {
  ...
  if (avcodec_parameters_to_context(stream_info->av_codec_ctx,
                                    stream_info->av_codec_params) < 0) {
  ...
  if (avcodec_open2(stream_info->av_codec_ctx, stream_info->av_codec, nullptr) <
      0) {
...
int VideoPlayer::init_sws_scaler_ctx(VideoState* video_state) {
  ...
  // Convert planar YUV 4:2:0 pixel format to packed RGB 8:8:8
  // pixel format with bilinear rescaling algorithm.
  sws_scaler_ctx = sws_getContext(
      width, height, stream_info->av_codec_ctx->pix_fmt, width, height,
...
int VideoPlayer::allocate_frame_buffer(AVPixelFormat pix_fmt,
                                       VideoDimension dimensions) {
  constexpr int LINESIZE_ALIGNMENT = 32;

  const auto buffer_size = av_image_get_buffer_size(
      AV_PIX_FMT_RGBA, dimensions.x, dimensions.y, LINESIZE_ALIGNMENT);

  const auto total_buffer_size = buffer_size * sizeof(std::uint8_t);

  m_video_state->buffer =
      static_cast<std::uint8_t*>(av_malloc(total_buffer_size));
...
int VideoPlayer::init_hwaccel_decoder(AVCodecContext* ctx,
...
AVPixelFormat VideoPlayer::get_hw_format(AVCodecContext* ctx,
...
int VideoPlayer::play_video(AVRational* timebase) {
  ...
  *timebase = video_stream_info->timebase;
  ...
  const int AV_STEREO_CHANNEL_NB =
...
unsigned int VideoPlayer::update_framebuffer(Uint32 interval, void* user_data) {
  ...
  std::array<std::uint8_t*, COLOR_CHANNELS_NB> dest = {data->buffer, nullptr,
                                                       nullptr, nullptr};

  std::array<int, COLOR_CHANNELS_NB> dest_linesize = {0, 0, 0, 0};
  dest_linesize[0] = data->dimensions.x * COLOR_CHANNELS_NB;

  sws_scale(sws_scaler_ctx, s_LatestFrame->data, s_LatestFrame->linesize, 0,
            data->dimensions.y, dest.data(), dest_linesize.data());
...
double VideoPlayer::calculateReferenceClock() {
  ...
  if (bytes_per_sec > 0) {
    ref_clock -=
        static_cast<double>(hw_buf_size) / static_cast<double>(bytes_per_sec);
  }

  return ref_clock;
}

double VideoPlayer::calculateActualDelay(VideoState* video_state,
                                         double& frame_timer) {
...
int VideoPlayer::decode_video_frame(VideoState* video_state,
  ...
  // Send the packet to the decoder
  int send_pkt_errcode =
      avcodec_send_packet(video_stream_info->av_codec_ctx, video_packet);
...
int VideoPlayer::seek_frame(float seconds) {
  ...
  // Perform the seeking operation on the audio stream first before the video stream.
  ...
  for (const std::string& key : stream_ids) {
    ...
    const auto target_timestamp = av_rescale_q(
        target_second, AVRational{1, AV_TIME_BASE}, stream_info->timebase);

    int response = av_seek_frame(av_format_ctx, stream_info->stream_index,
